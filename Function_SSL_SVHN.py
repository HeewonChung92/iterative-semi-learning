...
from datasets.imbalance_dataset_svhn import *
import models
from utils import *
from losses import *
...
def define_argument():
    ...
    parser.add_argument('--data_path', type=str, default='D:/img_SVHN/')
    ...
    parser.add_argument('--root_model', type=str, default='D:/img_SVHN/checkpoint/')
    parser.add_argument('--output_path', default='D:/img_SVHN/', type=str)
...
def load_dataset_pretrain(args):
    ...
    train_dataset = ImbalanceDataset_SVHN(
        ...
        rand_seed=args.rand_seed, split='train', download=True, transform=transform_train)

    unlb_dataset = datasets.SVHN(root=args.data_path, split='extra', download=True, transform=transform_val)
    val_dataset = datasets.SVHN(root=args.data_path, split='test', download=True, transform=transform_val)

    print('--------------------------------------------------------------------------------------------------------')
    aa, counts_train = np.unique(train_dataset.labels, return_counts=True)
    print('Distribution of classes in Original Train data: ', aa, counts_train, sum(counts_train))
    aa, counts_aa = np.unique(unlb_dataset.labels, return_counts=True)
    print('Distribution of classes in Unlabeled Train data: ', aa, counts_aa, sum(counts_aa))
    aa, counts_test = np.unique(val_dataset.labels, return_counts=True)
...
def load_dataset_semi(args, unlabeled_file):
    ...
    train_dataset = SemiSupervisedImbalance_SVHN(
        root=args.data_path, imb_type=args.imb_type, imb_factor=args.imb_factor,
        unlabel_imb_factor=args.imb_factor_unlabel, unlabeled_file=unlabeled_file,
        rand_seed=args.rand_seed, split='train', download=True, transform=transform_train
    )

    unlb_dataset = UnlabeledDatasetWithPseudoLabel_SVHN(root=args.data_path, unlabeled_file=unlabeled_file, transform=transform_val)
    val_dataset = datasets.SVHN(root=args.data_path, split='test', download=True, transform=transform_val)
    ...
    idx_stay = np.where(train_dataset.labels < 10)
    train_dataset.data = train_dataset.data[idx_stay, :, :, :].squeeze()
    train_dataset.labels = train_dataset.labels[idx_stay]
    aa, counts_train = np.unique(train_dataset.labels, return_counts=True)
    ...
    if args.train_rule == 'Resample':
        train_sampler = ImbalancedDatasetSampler(train_dataset)
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.workers, pin_memory=True, sampler=train_sampler)
    val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=100, shuffle=False)
    unlb_loader = torch.utils.data.DataLoader(unlb_dataset, batch_size=100, shuffle=False)
    return train_loader, unlb_loader, val_loader
